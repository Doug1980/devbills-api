...
import  prisma  from "../config/prisma"; // Ajuste o caminho conforme sua estrutura

type GlobalCategoryInput = Pick<Category, "name" | "color" | "type">; // ✅ "type" no singular
...
export const initializaGlobalCategories = async (): Promise<Category[]> => {
  ...
  for (const category of globalCategories) {
    try {
      const existing = await prisma.category.findFirst({
        where: {
          name: category.name,
          type: category.type, // ✅ "type" no singular
        },
      });

      if (!existing) {
        const newCategory = await prisma.category.create({ data: category });
        console.log(`Criada: ${newCategory.name}`); // ✅ Sem espaço no ${
